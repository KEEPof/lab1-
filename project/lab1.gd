...
@onready var check_box_3 = $ColorRect/VBoxContainer/HBoxContainer3/CheckBox3
...
func _ready():
	...
	check_box_3.toggled.connect(_on_check_box_3_toggled)
	print(button1)
...
func _on_button1_pressed():
	...
	if(check_box_3.button_pressed == true):
		text_output2.text = "Рекурсия 3: Вывод цифр"

		result = lab.recursion3(n);
		text_output1.text = result.get_digits_output()
		text_memory.text = "Памяти: " + str(result.get_memory_amount()) + " байт"
		text_time.text = "Время: " + str(result.get_time())
		text_count.text = "Шагов: " + str(result.get_calls())

		text_output3.text = "---"
		text_output_iter_mem.text = "---"
		text_time_iter.text = "---"
		text_count_iter.text = "---"
